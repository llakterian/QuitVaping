...
class NRTUsageChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (spots.length > 2) {
      ...
      final firstAvg = firstHalf.fold<double>(0.0, (sum, spot) => sum + (spot.y)) / (firstHalf.isEmpty ? 1 : firstHalf.length);
      final secondAvg = secondHalf.fold<double>(0.0, (sum, spot) => sum + (spot.y)) / (secondHalf.isEmpty ? 1 : secondHalf.length);
