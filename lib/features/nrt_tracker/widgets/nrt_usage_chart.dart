...
class NRTUsageChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (spots.length > 2) {
      ...
      final firstAvg = firstHalf.fold<double>(0.0, (sum, spot) => sum + ((spot.y is num ? (spot.y as num).toDouble() : 0.0))) / (firstHalf.isEmpty ? 1 : firstHalf.length);
      final secondAvg = secondHalf.fold<double>(0.0, (sum, spot) => sum + ((spot.y is num ? (spot.y as num).toDouble() : 0.0))) / (secondHalf.isEmpty ? 1 : secondHalf.length);
