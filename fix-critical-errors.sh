...
sed -i '346s/sum + craving.intensity/sum + (craving.intensity ?? 0)/g' lib/data/services/ai_service.dart
...
sed -i '239s/sum + record.nicotineStrength/sum + (record.nicotineStrength ?? 0.0)/g' lib/data/services/nrt_service.dart

# Fix NRT tracker screen error
echo "Fixing NRT tracker screen errors..."
sed -i '147s/sum + record.nicotineStrength/sum + (record.nicotineStrength ?? 0.0)/g' lib/features/nrt_tracker/screens/nrt_tracker_screen.dart

# Fix NRT usage chart errors
echo "Fixing NRT usage chart errors..."
sed -i '57s/sum + spot.y/sum + (spot.y ?? 0.0)/g' lib/features/nrt_tracker/widgets/nrt_usage_chart.dart
sed -i '58s/sum + spot.y/sum + (spot.y ?? 0.0)/g' lib/features/nrt_tracker/widgets/nrt_usage_chart.dart

# Fix craving analytics errors
echo "Fixing craving analytics errors..."
sed -i '364s/sum + entry.value/sum + (entry.value ?? 0)/g' lib/features/checkins/widgets/craving_analytics.dart
...
sed -i '370s/sum + entry.value/sum + (entry.value ?? 0)/g' lib/features/nrt_tracker/widgets/nrt_analytics_tab.dart
